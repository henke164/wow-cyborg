...
function RenderSingleTargetRotation()
  local nearbyEnemies = GetNearbyEnemyCount(5246);
  ...
  if (holActive) and nearbyEnemies > 0 then
  ...
  if InCombatLockdown() then
    if hpPercentage < 60 then
      ...
      if FindBuff("player", "Shield Wall") == nil then
  ...
  if nearbyEnemies > 0 then
    ...
    if IsCastableAtEnemyTarget("Shield Slam", 0) and rage < 70 then
      WowCyborg_CURRENTATTACK = "Shield Slam";
      return SetSpellRequest(shieldSlam);
    end

    if nearbyEnemies == 1 then
      if rage > 50 and targetHp < 20 and IsCastableAtEnemyTarget("Execute", 0) then
        WowCyborg_CURRENTATTACK = "Execute";
        return SetSpellRequest(execute);
      end
    end

    if IsCastable("Revenge", 70) and InMeleeRange() and (ipBuff ~= nil and ipTl > 4) then
      WowCyborg_CURRENTATTACK = "Revenge";
      return SetSpellRequest(revenge);
    end
  ...
  if IsCastable("Demoralizing Shout", 0) and (nearbyEnemies > 0 and rage < 70) then
